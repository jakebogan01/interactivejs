<script>
	...
	let code = `// Join colors1 with colors2
const colors1 = [];
const colors2 = [];
...
// `;
	...
	const colors1 = ['#f472b6', '#D163F9', '#0ea5e9'];
	const colors2 = ['#6963F9', '#F9F363', '#F98663'];

	const updateCode = () => {
		const colors1 = array1.map((item) => {
			const obj = array1.find((obj) => obj.id === item.id);
			return obj ? `"${obj.color}"` : '';
		});
		const colors2 = array2.map((item) => {
			const obj = array2.find((obj) => obj.id === item.id);
			return obj ? `"${obj.color}"` : '';
		});
		const test = colors1.concat(colors2);
		const children = test.map((item) => `${item}`);
		code = `// Join colors1 with colors2
const colors1 = [${colors1.join(', ')}];
const colors2 = [${colors2.join(', ')}];
const children = colors1.concat(colors2);
// ${children.join(', ')}`;
		createArray = hljs.highlight(code, { language: 'javascript' }).value;
	};

	const addBlock = (num, array) => {
		let newNumber;
		do {
			newNumber = Math.floor(Math.random() * 3);
		} while (array.some((item) => item.id === newNumber));
		if (num === 1) array.push({ id: newNumber, color: colors1[array.length % colors1.length] });
		if (num === 2) array.push({ id: newNumber, color: colors2[array.length % colors2.length] });
		updateCode();
		joinArrays();
	};

	const removeBlock = (array, id = null) => {
		if (array.length > 0) {
			if (id !== null) {
	...
	const joinArrays = () => (array3 = array1.concat(array2));
...
<h2 bind:this={titleEl} id="array-concat" class="text-white">Concat()</h2>

<p>
	The <span class="text-sky-500">concat()</span> method concatenates (joins) two or more arrays.
	<br />
	The <span class="text-sky-500">concat()</span> method returns a new array, containing the joined
	arrays.
	<br />
	The <span class="text-sky-500">concat()</span> method does not change the existing arrays.
</p>

<div class="mb-[38px] flex">
	<h3 class="min-w-[75px] text-sm text-gray-400">Colors 1:</h3>
	<div class="flex">
		<div class="mr-2 flex flex-col justify-center space-y-3">
			<button
				type="button"
				onclick={() => addBlock(1, array1)}
				aria-label="add"
			...
			<button
				type="button"
				onclick={() => removeBlock(array1)}
				aria-label="remove"
				class="cursor-pointer rounded-full border border-slate-700 bg-slate-800 text-gray-400 transition hover:bg-slate-600 hover:text-white disabled:border-slate-700 disabled:bg-slate-800 disabled:text-gray-400 disabled:opacity-50"
				disabled={array1.length === 0}
			>
				<svg
					xmlns="http://www.w3.org/2000/svg"
					viewBox="0 0 20 20"
		...
		<div class="relative">
			<div
				style="width: {array1.length * 60 + 11.6}px;"
			...
			>
				{#each array1 as item, index}
					<button
						...
						type="button"
						aria-label="block"
						onclick={() => removeBlock(array1, item.id)}
...
<div class="mb-[38px] flex">
	<h3 class="min-w-[75px] text-sm text-gray-400">Colors 2:</h3>
	<div class="flex">
		<div class="mr-2 flex flex-col justify-center space-y-3">
			<button
				type="button"
				onclick={() => addBlock(2, array2)}
				aria-label="add"
				class="cursor-pointer rounded-full border border-slate-700 bg-slate-800 text-gray-400 transition hover:bg-slate-600 hover:text-white disabled:border-slate-700 disabled:bg-slate-800 disabled:text-gray-400 disabled:opacity-50"
				disabled={array2.length >= 3}
			>
				<svg
					xmlns="http://www.w3.org/2000/svg"
					viewBox="0 0 20 20"
		...
		<div class="relative">
			<div
				style="width: {array2.length * 60 + 11.6}px;"
				class="flex min-h-[71.6px] min-w-[71.6px] space-x-2.5 overflow-hidden rounded-xl border border-slate-700 bg-slate-800 p-2.5 transition-[width] duration-300"
			>
				{#each array2 as item, index}
					<button
						...
						type="button"
						aria-label="block"
						onclick={() => removeBlock(array2, item.id)}
						class="group flex size-[50px] min-h-[50px] min-w-[50px] transform cursor-pointer items-center justify-center rounded-[5px] bg-pink-400 text-white transition duration-300 hover:scale-90 hover:!bg-red-500"
						style="background-color: {item.color};"
					>
						<svg
...
<div class="mb-[38px] flex min-h-[71.6px]">
	<h3 class="min-w-[75px] text-sm text-gray-400">Children:</h3>
	{#if array3.length > 0}
		<div class="relative">
			<div
				transition:scale|local={{ duration: 500 }}
				style="width: {array3.length * 60 + 11.6}px;"
				class="flex min-h-[71.6px] min-w-[71.6px] space-x-2.5 overflow-hidden rounded-xl border border-slate-700 bg-slate-800 p-2.5 transition-[width] duration-300"
			>
				{#each array3 as item, index}
					<div
						transition:scale|local={{ duration: 500 }}
			...
			<div
				transition:fade|local={{ duration: 150 }}
				class="absolute flex min-w-[71.6px] space-x-2.5 overflow-hidden px-2.5"
			>
				{#each array3 as item, index}
					<span
						transition:fade|local={{ duration: 150 }}
